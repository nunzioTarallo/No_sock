...
python3 cnn_inference_1.py $numero_monitor $tipo_dataset 

python3 preprocessing_1.py $preprocessing_debug_mode $mode $anomalous_dataset_type $n_monitor_layers $preprocessing_test_percentage $normalize $final_compression $final_compression_components 

cp Output/Inference/PP/Data/*  Input/Inference/AD/Data

python3 anomaly_detection_1.py $anomaly_detection_debug_mode $mode $ad_technique 
